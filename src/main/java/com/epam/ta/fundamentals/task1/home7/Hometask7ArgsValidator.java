package com.epam.ta.fundamentals.task1.home7;

import com.epam.ta.fundamentals.task1.exception.ArgsValidationException;
import com.epam.ta.fundamentals.task1.validator.AbstractArgsValidator;

public class Hometask7ArgsValidator extends AbstractArgsValidator {

	private static final String WRONG_ARGS_AMOUNT = "Wrong amount of arguments.";
	private static final String WRONG_NUMBER_FORMAT = "Argument should be an integer number.";
	private static final String NEGATIVE_STEP = "Step can't be a negative";
	private static int ARGS_AMOUNT = 3;

	public Hometask7ArgsValidator(String[] argsString) {
		super(argsString);
	}

	@Override
	public void checkArguments() throws ArgsValidationException {
		validateAmountOfArgsEqualTo(ARGS_AMOUNT, WRONG_ARGS_AMOUNT);
		extractDoubleArguments(WRONG_NUMBER_FORMAT);
		checkStepIsPositive();
	}

	
	public void checkStepIsPositive() throws ArgsValidationException {
		if (getDoubleArgsArr(2) <= 0) {
			throw new ArgsValidationException(NEGATIVE_STEP);
		}
	}
}
